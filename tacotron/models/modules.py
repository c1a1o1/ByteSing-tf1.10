...
class MultiBiLSTMLayer:
	"""Duration model bidirectional multiple layer LSTM
	"""
	def __init__(self, is_training, size=256, zoneout=0.1, scope=None, LSTM_layers=4):
		"""
		Args:
			...
			scope: duration model scope.
		"""
		super(MultiBiLSTMLayer, self).__init__()
		self.is_training = is_training

		self.size = size
		self.zoneout = zoneout
		self.scope = 'duration_LSTM' if scope is None else scope

		#Create forward LSTM Cell
		self._fw_hidden_layers = [ZoneoutLSTMCell(size, is_training,
			zoneout_factor_cell=zoneout,
			zoneout_factor_output=zoneout,
			name='duration_fw_LSTM{}'.format(i+1)) for i in range(LSTM_layers)]
		self._fw_cell = tf.nn.rnn_cell.MultiRNNCell(cells=self._fw_hidden_layers, state_is_tuple=True)
		
		#Create backward LSTM Cell
		self._bw_hidden_layers = [ZoneoutLSTMCell(size, is_training,
			zoneout_factor_cell=zoneout,
			zoneout_factor_output=zoneout,
			name='duration_bw_LSTM{}'.format(i+1)) for i in range(LSTM_layers)]
		self._bw_cell = tf.nn.rnn_cell.MultiRNNCell(cells=self._bw_hidden_layers, state_is_tuple=True)
